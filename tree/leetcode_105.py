...
class Solution:
    ...
    def buildTree(self, preorder: list[int], inorder: list[int]) -> TreeNode | None:
        ...
        def build(inorder_start: int, inorder_end: int) -> TreeNode | None:
            ...
            # 5. 创建根节点：当前子树的根是前序数组中 self.preorder_index 指向的元素
            root_val = preorder[self.preorder_index]  # 使用外层函数的 preorder
            root = TreeNode(root_val)

            # 6. 更新前序索引：指向下一个需要处理的根节点
            self.preorder_index += 1

            # 7. 找到根节点在中序数组中的位置
            root_inorder_index = self.inorder_map[root_val]

            # 8. 递归构建左子树 (在中序数组中，根节点左边的部分 [inorder_start, root_inorder_index - 1])
            # 注意这里先构建左子树，是因为前序遍历的顺序是 根 -> 左 -> 右
            root.left = build(inorder_start, root_inorder_index - 1)

            # 9. 递归构建右子树 (在中序数组中，根节点右边的部分 [root_inorder_index + 1, inorder_end])
...
class TestBuildTree(unittest.TestCase):
    ...
    def test_example_1(self):
        ...
        solution = Solution()
        actual_root = solution.buildTree(preorder, inorder)
    ...
    def test_two_nodes_left_child(self):
        # 两个节点，左孩子
        # preorder = [1, 2], inorder = [2, 1] -> 1 是根，2 在 1 左边 (中序)，2 在 1 后面 (前序)，所以 2 是 1 的左孩子
        preorder = [1, 2]
        inorder = [2, 1]
    ...
    def test_two_nodes_right_child(self):
        # 两个节点，右孩子
        # preorder = [1, 2], inorder = [1, 2] -> 1 是根，2 在 1 右边 (中序)，2 在 1 后面 (前序)，所以 2 是 1 的右孩子
        preorder = [1, 2]
        inorder = [1, 2]
    ...
    def test_simple_tree(self):
        ...
        # Root 3. Inorder [1,2] | 3 | [4]. Left pre [1,2], Right pre [4]
        # Left from pre=[1,2], in=[1,2]: Root 1. Inorder [1] | 2. Left empty, Right [2]. Pre left [], right [2]. Left of 1 None. Right of 1 from pre=[2], in=[2] -> Root 2. Left/Right empty. Tree: 1 -> right 2.
        # Right from pre=[4], in=[4]: Root 4. Left/Right empty. Tree: 4.
        # Final: 3 -> left (1->right 2), right 4
        # 结构:   3
        #        / \
        #       1   4
        #        \
    ...
    def test_complex_tree(self):
        # 复杂一些的树
        # preorder = [4, 2, 1, 3, 5], inorder = [1, 2, 3, 4, 5]
        # Root 4. Inorder [1,2,3] | 4 | [5]. Left pre [2,1,3], Right pre [5]
        # Left from pre=[2,1,3], in=[1,2,3]: Root 2. Inorder [1] | 2 | [3]. Left pre [1], Right pre [3]
        #   Left from pre=[1], in=[1]: Root 1. Returns node(1). Left of 2 is 1.
        #   Right from pre=[3], in=[3]: Root 3. Returns node(3). Right of 2 is 3.
        #   Tree from [2,1,3], [1,2,3]: 2 -> left 1, right 3.
        # Right from pre=[5], in=[5]: Root 5. Returns node(5). Right of 4 is 5.
        # Final: 4 -> left (2->left 1, right 3), right 5
        # 结构:   4
        #        / \
        #       2   5
        #      / \
